<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h1 class="text-2xl font-bold text-slate-900">{{ Auth::user()->rol === 'admin' ? 'Gestión de PQRS' : 'Mis PQRS' }}</h1><p class="mt-1 text-sm text-slate-500">Consulta y administra las solicitudes registradas.</p></div><a href="{{ route('pqrs.create') }}" class="inline-flex items-center justify-center rounded-lg bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-indigo-700"><span class="mr-2 text-lg leading-none">+</span> Nueva PQRS</a></div>
    </x-slot>
    ...
    <div class="space-y-6 p-4 sm:p-6 lg:p-8">
        ...
        <div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
            <div class="overflow-x-auto"><table class="min-w-full divide-y divide-slate-200 text-sm"><thead class="bg-slate-50 text-left text-xs font-semibold uppercase tracking-wide text-slate-500"><tr><th class="px-5 py-3.5">Radicado / asunto</th><th class="px-5 py-3.5">Categoría</th><th class="px-5 py-3.5">Estado</th><th class="px-5 py-3.5">Fechas</th>@if(Auth::user()->rol === 'admin')<th class="px-5 py-3.5">Residente</th>@endif<th class="px-5 py-3.5 text-right">Acciones</th></tr></thead>
                ...
                @forelse ($pqrs as $pqr)
                    @php $estado = match ($pqr->estado) {'radicada' => ['Radicada','bg-green-100 text-green-800'], 'en_revision' => ['En revisión','bg-yellow-100 text-yellow-800'], 'en_proceso' => ['En proceso','bg-violet-100 text-violet-800'], 'en_espera' => ['En espera','bg-amber-100 text-amber-800'], 'rechazada' => ['Rechazada','bg-rose-100 text-rose-800'], 'resuelta' => ['Resuelta','bg-orange-100 text-orange-800'], 'cerrada' => ['Cerrada','bg-blue-100 text-blue-800'], default => [ucfirst($pqr->estado),'bg-slate-100 text-slate-700']}; @endphp
                    <tr class="hover:bg-slate-50/70"><td class="px-5 py-4"><p class="font-semibold text-slate-900">#{{ $pqr->id }} · {{ $pqr->asunto }}</p><p class="mt-1 max-w-xs truncate text-xs text-slate-500">{{ $pqr->descripcion }}</p></td><td class="px-5 py-4 text-slate-600">{{ $pqr->tipoPqr?->nombre ?? 'Sin categoría' }}</td><td class="px-5 py-4"><span class="inline-flex rounded-full px-2.5 py-1 text-xs font-semibold {{ $estado[1] }}">{{ $estado[0] }}</span></td><td class="whitespace-nowrap px-5 py-4"><p class="text-slate-700">{{ $pqr->fecha_radicacion->format('d/m/Y') }}</p><p class="mt-1 text-xs {{ $pqr->fecha_limite_respuesta->isPast() && !in_array($pqr->estado, \App\Models\Pqr::inactiveStatuses(), true) ? 'font-semibold text-rose-600' : 'text-slate-400' }}">Límite: {{ $pqr->fecha_limite_respuesta->format('d/m/Y') }}</p></td>@if(Auth::user()->rol === 'admin')<td class="px-5 py-4 text-slate-600">{{ $pqr->user?->name ?? 'Sin usuario' }}</td>@endif<td class="px-5 py-4"><div class="flex justify-end gap-3"><a href="{{ route('pqrs.edit', $pqr) }}" class="font-semibold text-indigo-600 hover:text-indigo-800">{{ Auth::user()->rol === 'admin' ? 'Gestionar' : 'Ver detalle' }}</a>@can('delete', $pqr)<form action="{{ route('pqrs.destroy', $pqr) }}" method="POST" onsubmit="return confirm('¿Eliminar esta PQRS?')">@csrf @method('DELETE')<button class="font-semibold text-rose-600 hover:text-rose-800">Eliminar</button></form>@endcan</div></td></tr>
                @empty <tr><td colspan="6" class="px-5 py-14 text-center"><p class="font-medium text-slate-600">No hay PQRS registradas</p><p class="mt-1 text-sm text-slate-400">Crea una nueva solicitud para comenzar.</p></td></tr> @endforelse
                </tbody></table></div>
