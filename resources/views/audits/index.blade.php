<x-app-layout>
    ...
    <div class="space-y-6 p-4 sm:p-6 lg:p-8">
        <div class="grid grid-cols-2 gap-3 lg:grid-cols-4">
            @foreach ([['Eventos registrados',$stats['total'],'border-indigo-200 bg-indigo-50 text-indigo-700'],['Eventos de hoy',$stats['today'],'border-emerald-200 bg-emerald-50 text-emerald-700'],['Usuarios identificados',$stats['users'],'border-violet-200 bg-violet-50 text-violet-700'],['PQRS auditadas',$stats['pqrs'],'border-sky-200 bg-sky-50 text-sky-700']] as $card)<div class="rounded-xl border p-4 shadow-sm {{ $card[2] }}"><p class="text-xs font-semibold uppercase tracking-wide opacity-80">{{ $card[0] }}</p><p class="mt-2 text-2xl font-bold">{{ $card[1] }}</p></div>@endforeach
        </div>
        ...
        <form method="GET" action="{{ route('audits.index') }}" class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                ...
                <select name="module" class="rounded-lg border-slate-300 text-sm focus:border-indigo-500 focus:ring-indigo-500"><option value="">Todos los módulos</option>@foreach($modules as $module)<option value="{{ $module }}" @selected(request('module') === $module)>{{ $module === 'PQR' ? 'PQRS' : $module }}</option>@endforeach</select>
