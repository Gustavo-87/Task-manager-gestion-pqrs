<x-app-layout>
    <x-slot name="header">
        <div><h1 class="text-2xl font-bold text-slate-900">Panel principal</h1><p class="mt-1 text-sm text-slate-500">Resumen general de las PQR y sus vencimientos.</p></div>
    </x-slot>

    <div class="space-y-6 p-4 sm:p-6 lg:p-8">
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
            @foreach ([
                ['label' => 'Total de PQR', 'value' => $counts->total ?? 0, 'card' => 'border-indigo-200 bg-indigo-50', 'text' => 'text-indigo-700', 'iconBg' => 'bg-indigo-100 text-indigo-700', 'icon' => 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l3.414 3.414A1 1 0 0117 7.414V19a2 2 0 01-2 2z'],
                ...
                ['label' => 'Respondidas', 'value' => $counts->respondidas ?? 0, 'card' => 'border-orange-200 bg-orange-50', 'text' => 'text-orange-700', 'iconBg' => 'bg-orange-100 text-orange-700', 'icon' => 'M5 13l4 4L19 7'],
                ...
                ['label' => 'Vencidas', 'value' => $overdue, 'card' => 'border-rose-200 bg-rose-50', 'text' => 'text-rose-700', 'iconBg' => 'bg-rose-100 text-rose-700', 'icon' => 'M12 9v2m0 4h.01M5.07 19h13.86c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.338 16c-.77 1.333.192 3 1.732 3z'],
            ] as $card)
        ...
        <div class="grid gap-6 xl:grid-cols-3">
            <section class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm xl:col-span-2">
                ...
                <div class="divide-y divide-slate-100">
                    ...
                    @empty <p class="px-5 py-10 text-center text-sm text-slate-500">No hay PQR registradas.</p> @endforelse
                </div>
            </section>
            ...
            <section class="rounded-xl border border-slate-200 bg-white shadow-sm">
                ...
                <div class="divide-y divide-slate-100">
                    @forelse ($upcoming as $pqr)
                        <div class="px-5 py-4"><p class="text-sm font-semibold text-slate-800">#{{ $pqr->id }} · {{ $pqr->asunto }}</p><p class="mt-1 text-xs font-medium text-rose-600">Vence {{ $pqr->fecha_limite_respuesta->isToday() ? 'hoy' : 'mañana' }}</p></div>
                    @empty <div class="px-5 py-10 text-center"><p class="text-sm font-medium text-slate-600">Sin alertas próximas</p><p class="mt-1 text-xs text-slate-400">No hay PQR por vencer hoy o mañana.</p></div> @endforelse
                </div>
            </section>
        </div>

        <div class="flex flex-wrap gap-3">
            <a href="{{ route('pqrs.create') }}" class="rounded-lg bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-indigo-700">Nueva PQR</a>
            <a href="{{ route('pqrs.index') }}" class="rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50">Consultar PQR</a>
