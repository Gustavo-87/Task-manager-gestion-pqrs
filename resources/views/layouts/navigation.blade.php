@php
    $items = [
        ...
        ['label' => Auth::user()->rol === 'admin' ? 'Gestión de PQR' : 'Mis PQR', 'route' => 'pqrs.index', 'active' => 'pqrs.*', 'icon' => 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l3.414 3.414A1 1 0 0117 7.414V19a2 2 0 01-2 2z'],
    ];
...
<aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-50 flex w-64 flex-col bg-slate-900 text-white transition-transform duration-200 lg:translate-x-0">
    <div class="flex h-20 items-center gap-3 border-b border-slate-800 px-5">
        <x-application-logo class="h-10 w-10 rounded-lg bg-white object-contain p-1 shadow-none" />
        <div><p class="font-bold tracking-tight">Gestión PQRS</p><p class="text-xs text-slate-400">Conjunto residencial</p></div>
    ...
    <nav class="flex-1 space-y-1 overflow-y-auto px-3 py-5">
        ...
        @if (Auth::user()->rol !== 'admin')
            <a href="{{ route('pqrs.create') }}" class="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-slate-300 hover:bg-slate-800 hover:text-white">
                <span class="flex h-5 w-5 items-center justify-center text-xl">+</span> Nueva PQR
            </a>
        @endif
    </nav>
