<x-app-layout>
    <x-slot name="header"><div><h1 class="text-2xl font-bold text-slate-900">Reportes de PQRS</h1><p class="mt-1 text-sm text-slate-500">Analiza la gestión y genera documentos PDF por periodo.</p></div></x-slot>
    ...
    <div class="space-y-6 p-8">
        ...
        <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
            @foreach ([['PQRS del periodo',$preview['pqrs']->count(),'border-indigo-200 bg-indigo-50 text-indigo-700'],['Vencidas',$preview['overdue']->count(),'border-rose-200 bg-rose-50 text-rose-700'],['Próximas a vencer',$preview['upcoming']->count(),'border-amber-200 bg-amber-50 text-amber-700'],['Porcentaje atendido',$attendedPercentage.'%','border-emerald-200 bg-emerald-50 text-emerald-700']] as $card)<div class="rounded-xl border p-5 shadow-sm {{ $card[2] }}"><p class="text-xs font-semibold uppercase tracking-wide opacity-80">{{ $card[0] }}</p><p class="mt-2 text-3xl font-bold">{{ $card[1] }}</p></div>@endforeach
        </div>

        <div class="grid grid-cols-2 gap-6">
            <section class="rounded-xl border border-slate-200 bg-white shadow-sm"><div class="border-b border-slate-100 px-5 py-4"><h2 class="font-semibold text-slate-900">Resumen por estado</h2><p class="mt-1 text-xs text-slate-500">Distribución de las PQRS incluidas en el periodo.</p></div><div class="space-y-4 p-5">
                @foreach(['radicada' => ['Radicadas','bg-green-500','text-green-700'],'en_revision' => ['En revisión','bg-yellow-500','text-yellow-700'],'en_proceso' => ['En proceso','bg-violet-500','text-violet-700'],'en_espera' => ['En espera','bg-amber-500','text-amber-700'],'rechazada' => ['Rechazadas','bg-rose-500','text-rose-700'],'resuelta' => ['Resueltas','bg-orange-500','text-orange-700'],'cerrada' => ['Cerradas','bg-blue-500','text-blue-700']] as $status => [$label,$bar,$text])
        ...
        <section class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm"><div class="border-b border-slate-100 px-5 py-4"><h2 class="font-semibold text-slate-900">Vista previa de PQRS</h2><p class="mt-1 text-xs text-slate-500">Primeros 10 registros que aparecerán en el reporte.</p></div><table class="min-w-full divide-y divide-slate-200 text-sm"><thead class="bg-slate-50 text-left text-xs font-semibold uppercase tracking-wide text-slate-500"><tr><th class="px-5 py-3">Radicado</th><th class="px-5 py-3">Asunto</th><th class="px-5 py-3">Categoría</th><th class="px-5 py-3">Estado</th><th class="px-5 py-3">Fecha límite</th></tr></thead><tbody class="divide-y divide-slate-100">@forelse($preview['pqrs']->take(10) as $pqr)<tr><td class="px-5 py-3 font-semibold text-slate-700">#{{ $pqr->id }}</td><td class="max-w-sm truncate px-5 py-3 text-slate-700">{{ $pqr->asunto }}</td><td class="px-5 py-3 text-slate-500">{{ $pqr->tipoPqr?->nombre ?? 'Sin categoría' }}</td><td class="px-5 py-3 text-slate-600">{{ \App\Models\Pqr::statusLabel($pqr->estado) }}</td><td class="px-5 py-3 text-slate-600">{{ $pqr->fecha_limite_respuesta->format('d/m/Y') }}</td></tr>@empty<tr><td colspan="5" class="px-5 py-10 text-center text-slate-500">No hay PQRS en este periodo.</td></tr>@endforelse</tbody></table></section>
